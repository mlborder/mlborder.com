Event.create([
  { name: '開幕！アイドルイントロダクション',
    event_type: 'unknown_event',
    started_at: '2013-03-05 17:00:00 +0900',
    ended_at: '2013-03-11 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 23 }, # 野々原茜
    ]
  },
  { name: '開演！ロケットスタートライブ!',
    event_type: 'unknown_event',
    started_at: '2013-03-14 17:00:00 +0900',
    ended_at: '2013-03-25 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 5 }, # 高槻やよい
    ]
  },
  { name: '輝け！春の野外音楽フェスティバル',
    event_type: 'unknown_event',
    started_at: '2013-03-28 17:00:00 +0900',
    ended_at: '2013-04-08 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 8 }, # 四条貴音
    ]
  },
  { name: 'ようこそ！アイドル学園天国',
    event_type: 'unknown_event',
    started_at: '2013-04-11 17:00:00 +0900',
    ended_at: '2013-04-22 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 6 }, # 菊地真
    ]
  },
  { name: '風雲！アイドルキャッスル！',
    event_type: 'unknown_event',
    started_at: '2013-04-25 17:00:00 +0900',
    ended_at: '2013-05-07 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 7 }, # 水瀬伊織
    ]
  },
  { name: '大爆走！765レーサーグランプリ',
    event_type: 'unknown_event',
    started_at: '2013-05-09 17:00:00 +0900',
    ended_at: '2013-05-20 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 27 }, # 高山紗代子
    ]
  },
  { name: '輝け！アイドルライブバトルアリーナ',
    event_type: 'unknown_event',
    started_at: '2013-05-23 17:00:00 +0900',
    ended_at: '2013-05-27 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 19 }, # 佐竹美奈子
    ]
  },
  { name: '大合戦！戦国アイドル活劇',
    event_type: 'unknown_event',
    started_at: '2013-05-29 17:00:00 +0900',
    ended_at: '2013-06-10 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 2 }, # 如月千早
    ]
  },
  { name: '激闘！NAMCOアイドルサッカーフェス2013',
    event_type: 'unknown_event',
    started_at: '2013-06-13 17:00:00 +0900',
    ended_at: '2013-06-24 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 13 }, # 我那覇響
    ]
  },
  { name: '夏直前！アイドル水上大運動会！',
    event_type: 'unknown_event',
    started_at: '2013-06-27 17:00:00 +0900',
    ended_at: '2013-07-08 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 35 }, # 木下ひなた
    ]
  },
  { name: '納涼！アイドル夏祭り',
    event_type: 'unknown_event',
    started_at: '2013-07-11 17:00:00 +0900',
    ended_at: '2013-07-22 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 10 }, # 三浦あずさ
    ]
  },
  { name: '白熱！アイドルユニットトーナメント',
    event_type: 'unknown_event',
    started_at: '2013-07-25 17:00:00 +0900',
    ended_at: '2013-07-30 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 36 }, # 矢吹可奈
    ]
  },
  { name: '熱唱！真夏のサマーライブフェス',
    event_type: 'unknown_event',
    started_at: '2013-08-02 17:00:00 +0900',
    ended_at: '2013-08-12 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 25 }, # ロコ
    ]
  },
  { name: '大激闘！765プロ野球',
    event_type: 'unknown_event',
    started_at: '2013-08-15 17:00:00 +0900',
    ended_at: '2013-08-26 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 47 }, # 永吉昴
    ]
  },
  { name: '恐怖！アイドルドキドキ肝だめし',
    event_type: 'unknown_event',
    started_at: '2013-08-29 14:00:00 +0900',
    ended_at: '2013-09-09 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 8 }, # 四条貴音
    ]
  },
  { name: '遊べ！アイドルゲームフェス',
    event_type: 'unknown_event',
    started_at: '2013-09-12 17:00:00 +0900',
    ended_at: '2013-09-24 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 7 }, # 水瀬伊織
    ]
  },
  { name: '大航海！アイドルパイレーツ',
    event_type: 'unknown_event',
    started_at: '2013-09-26 14:00:00 +0900',
    ended_at: '2013-10-07 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 11 }, # 双海亜美
    ]
  },
  { name: 'アイドルマスターズカップ',
    event_type: 'imc_event',
    started_at: '2013-10-10 17:00:00 +0900',
    ended_at: '2013-10-15 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 15 }, # 最上静香
    ]
  },
  { name: '開催！アイドル学園文化祭',
    event_type: 'choco_event',
    started_at: '2013-10-17 17:00:00 +0900',
    ended_at: '2013-10-28 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 31 }, # 天空橋朋花
    ]
  },
  { name: '狂騒！ハロウィンフェスタ',
    event_type: 'raid_event',
    started_at: '2013-10-30 14:00:00 +0900',
    ended_at: '2013-11-12 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 41 }, # 豊川風花
    ]
  },
  { name: 'GO!GO! アイドルゴルフ大会',
    event_type: 'unknown_event',
    started_at: '2013-11-14 17:00:00 +0900',
    ended_at: '2013-11-25 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 9 }, # 秋月律子
    ]
  },
  { name: '極めよ！アイドル道',
    event_type: 'lesson_event',
    started_at: '2013-11-28 17:00:00 +0900',
    ended_at: '2013-12-09 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 1 }, # 天海春香
    ]
  },
  { name: 'アイドルマスターズカップ2',
    event_type: 'imc_event',
    started_at: '2013-12-12 17:00:00 +0900',
    ended_at: '2013-12-16 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 6 }, # 菊地真
    ]
  },
  { name: '聖夜の奇跡！アイドルクリスマスTV',
    event_type: 'lesson_event',
    started_at: '2013-12-17 17:00:00 +0900',
    ended_at: '2013-12-25 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 11 }, # 双海亜美
    ]
  },
  { name: '年またぎ！アイドル紅白歌祭り',
    event_type: 'raid_event',
    started_at: '2013-12-26 17:00:00 +0900',
    ended_at: '2014-01-06 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 50 }, # ジュリア
    ]
  },
  { name: '招福！アイドル干支レース',
    event_type: 'raid_event',
    started_at: '2014-01-09 17:00:00 +0900',
    ended_at: '2014-01-21 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 20 }, # 所恵美
    ]
  },
  { name: '出演！アイドルスペースウォーズ',
    event_type: 'lesson_event',
    started_at: '2014-01-23 17:00:00 +0900',
    ended_at: '2014-02-04 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 13 }, # 我那覇響
    ]
  },
  { name: 'アイドルマスターズカップ3',
    event_type: 'imc_event',
    started_at: '2014-02-06 17:00:00 +0900',
    ended_at: '2014-02-10 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 49 }, # 周防桃子
    ]
  },
  { name: '甘いご奉仕！バレンタインキャラバン',
    event_type: 'choco_event',
    started_at: '2014-02-13 17:00:00 +0900',
    ended_at: '2014-02-24 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 17 }, # 田中琴葉
    ]
  },
  { name: '出撃！アイドルヒーローズ',
    event_type: 'raid_event',
    started_at: '2014-02-27 17:00:00 +0900',
    ended_at: '2014-03-10 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 26 }, # 七尾百合子
    ]
  },
  { name: '大合奏！アイドルシンフォニー',
    event_type: 'lesson_event',
    started_at: '2014-03-13 17:00:00 +0900',
    ended_at: '2014-03-24 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 4 }, # 萩原雪歩
    ]
  },
  { name: '集まれ！アイドルモンスターズ',
    event_type: 'raid_event',
    started_at: '2014-03-27 17:00:00 +0900',
    ended_at: '2014-04-02 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 29 }, # 高坂海美
    ]
  },
  { name: '【復活】ようこそ！アイドル学園天国',
    event_type: 'choco_event',
    started_at: '2014-04-04 17:00:00 +0900',
    ended_at: '2014-04-08 16:59:59 +0900'
  },
  { name: 'アイドルマスターズカップ4',
    event_type: 'imc_event',
    started_at: '2014-04-10 17:00:00 +0900',
    ended_at: '2014-04-14 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 40 }, # 大神環
    ]
  },
  { name: '開幕！春のサイクリングレース',
    event_type: 'choco_event',
    started_at: '2014-04-17 17:00:00 +0900',
    ended_at: '2014-04-28 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 6 }, # 菊地真
    ]
  },
  { name: '逆襲！グレートアイドルキャッスル',
    event_type: 'raid_event',
    started_at: '2014-04-30 17:00:00 +0900',
    ended_at: '2014-05-07 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 2 }, # 如月千早
    ]
  },
  { name: '復活！NAMCOアイドルサッカーフェス2013',
    event_type: 'choco_event',
    started_at: '2014-05-09 17:00:00 +0900',
    ended_at: '2014-05-13 16:59:59 +0900'
  },
  { name: '夢いっぱい！メルヘンアイドル物語',
    event_type: 'lesson_event',
    started_at: '2014-05-15 17:00:00 +0900',
    ended_at: '2014-05-26 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 22 }, # 箱崎星梨花
    ]
  },
  { name: '大戦乱！アイドル三国志',
    event_type: 'raid_event',
    started_at: '2014-05-29 17:00:00 +0900',
    ended_at: '2014-06-04 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 3 }, # 星井美希
    ]
  },
  { name: '集え！NAMCOアイドルサッカーフェス2014',
    event_type: 'choco_event',
    started_at: '2014-06-06 17:00:00 +0900',
    ended_at: '2014-06-17 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 16 }, # 伊吹翼
    ]
  },
  { name: 'アイドルマスターズカップ5',
    event_type: 'imc_event',
    started_at: '2014-06-19 17:00:00 +0900',
    ended_at: '2014-06-23 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 7 }, # 水瀬伊織
    ]
  },
  { name: '進軍！アイドルフォース',
    event_type: 'raid_event',
    started_at: '2014-06-25 17:00:00 +0900',
    ended_at: '2014-07-02 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 44 }, # 真壁瑞希
    ]
  },
  { name: '極限！サバイバルアイランド',
    event_type: 'lesson_event',
    started_at: '2014-07-04 17:00:00 +0900',
    ended_at: '2014-07-15 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 11 }, # 双海亜美
    ]
  },
  { name: '決行！プラチナスターライブ1ST',
    event_type: 'psl_event',
    started_at: '2014-07-17 17:00:00 +0900',
    ended_at: '2014-07-24 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 13 }, # 我那覇響
    ]
  },
  { name: '決行！プラチナスターライブ1STアンコール',
    event_type: 'psl_event',
    started_at: '2014-07-25 17:00:00 +0900',
    ended_at: '2014-07-28 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 14 }, # 春日未来
    ]
  },
  { name: 'HAPPY★HAPPY★PERFORM@NCE',
    event_type: 'hhp_event',
    started_at: '2014-07-30 17:00:00 +0900',
    ended_at: '2014-08-06 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 14 }, # 春日未来
      { idol_id: 15 }, # 最上静香
      { idol_id: 16 }, # 伊吹翼
      { idol_id: 17 }, # 田中琴葉
      { idol_id: 18 }, # 島原エレナ
      { idol_id: 19 }, # 佐竹美奈子
      { idol_id: 20 }, # 所恵美
      { idol_id: 21 }, # 徳川まつり
      { idol_id: 22 }, # 箱崎星梨花
      { idol_id: 23 }, # 野々原茜
      { idol_id: 24 }, # 望月杏奈
      { idol_id: 25 }, # ロコ
      { idol_id: 26 }, # 七尾百合子
      { idol_id: 27 }, # 高山紗代子
      { idol_id: 28 }, # 松田亜利沙
      { idol_id: 29 }, # 高坂海美
      { idol_id: 30 }, # 中谷育
      { idol_id: 31 }, # 天空橋朋花
      { idol_id: 32 }, # エミリー
      { idol_id: 33 }, # 北沢志保
      { idol_id: 34 }, # 舞浜歩
      { idol_id: 35 }, # 木下ひなた
      { idol_id: 36 }, # 矢吹可奈
      { idol_id: 37 }, # 横山奈緒
      { idol_id: 38 }, # 二階堂千鶴
      { idol_id: 39 }, # 馬場このみ
      { idol_id: 40 }, # 大神環
      { idol_id: 41 }, # 豊川風花
      { idol_id: 42 }, # 宮尾美也
      { idol_id: 43 }, # 福田のり子
      { idol_id: 44 }, # 真壁瑞希
      { idol_id: 45 }, # 篠宮可憐
      { idol_id: 46 }, # 百瀬莉緒
      { idol_id: 47 }, # 永吉昴
      { idol_id: 48 }, # 北上麗花
      { idol_id: 49 }, # 周防桃子
      { idol_id: 50 }, # ジュリア
    ]
  },
  { name: '納涼！アイドル夏祭りin港町',
    event_type: 'choco_event',
    started_at: '2014-08-08 17:00:00 +0900',
    ended_at: '2014-08-19 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 21 }, # 徳川まつり
    ]
  },
  { name: 'アイドルマスターズカップ6',
    event_type: 'imc_event',
    started_at: '2014-08-21 17:00:00 +0900',
    ended_at: '2014-08-25 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 50 }, # ジュリア
    ]
  },
  { name: '戦慄！アイドル肝だめし病棟',
    event_type: 'raid_event',
    started_at: '2014-08-27 17:00:00 +0900',
    ended_at: '2014-09-03 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 48 }, # 北上麗花
    ]
  },
  { name: '夢いっぱい！アイドルゲームフェス2014',
    event_type: 'lesson_event',
    started_at: '2014-09-05 17:00:00 +0900',
    ended_at: '2014-09-16 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 41 }, # 豊川風花
    ]
  },
  { name: '結実！プラチナスターライブ2ND',
    event_type: 'psl_event',
    started_at: '2014-09-17 17:00:00 +0900',
    ended_at: '2014-09-28 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 2 }, # 如月千早
      { idol_id: 15 }, # 最上静香
    ]
  },
  { name: '決選！学園ミスコンサバイバル',
    event_type: 'raid_event',
    started_at: '2014-09-30 17:00:00 +0900',
    ended_at: '2014-10-08 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 42 }, # 宮尾美也
    ]
  },
  { name: '昂れ！アイドルファイト',
    event_type: 'choco_event',
    started_at: '2014-10-10 17:00:00 +0900',
    ended_at: '2014-10-20 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 43 }, # 福田のり子
    ]
  },
  { name: 'アイドルマスターズカップ7',
    event_type: 'imc_event',
    started_at: '2014-10-22 17:00:00 +0900',
    ended_at: '2014-10-26 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 37 }, # 横山奈緒
    ]
  },
  { name: '激動！アイドル警察24時',
    event_type: 'raid_event',
    started_at: '2014-10-28 17:00:00 +0900',
    ended_at: '2014-11-05 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 49 }, # 周防桃子
    ]
  },
  { name: '進出！アジアンライブツアー',
    event_type: 'lesson_event',
    started_at: '2014-11-07 17:00:00 +0900',
    ended_at: '2014-11-16 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 20 }, # 所恵美
    ]
  },
  { name: '熱烈！プラチナスターライブ3RD',
    event_type: 'psl_event',
    started_at: '2014-11-17 17:00:00 +0900',
    ended_at: '2014-11-26 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 1 }, # 天海春香
      { idol_id: 17 }, # 田中琴葉
    ]
  },
  { name: '祝祭！クリスマスフェスタ',
    event_type: 'choco_event',
    started_at: '2014-11-28 17:00:00 +0900',
    ended_at: '2014-12-08 16:59:59 +0900',
    prizes_attributes: [
      { idol_id: 34 }, # 舞浜歩
    ]
  },
  { name: 'アイドルマスターズカップ8',
    event_type: 'imc_event',
    started_at: '2014-12-10 17:00:00 +0900',
    ended_at: '2014-12-14 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 24 }, # 望月杏奈
    ]
  },
  { name: '神vs魔！ホーリーナイトラウンド',
    event_type: 'raid_event',
    started_at: '2014-12-16 17:00:00 +0900',
    ended_at: '2014-12-24 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 4 }, # 萩原雪歩
    ]
  },
  { name: '年越し！アイドル美食グランプリ',
    event_type: 'lesson_event',
    started_at: '2014-12-26 17:00:00 +0900',
    ended_at: '2015-01-05 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 10 }, # 三浦あずさ
    ]
  },
  { name: '招福！アイドル干支マラソン',
    event_type: 'raid_event',
    started_at: '2015-01-07 17:00:00 +0900',
    ended_at: '2015-01-14 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 36 }, # 矢吹可奈
    ]
  },
  { name: '大成！プラチナスターライブ4TH',
    event_type: 'psl_event',
    started_at: '2015-01-16 17:00:00 +0900',
    ended_at: '2015-01-27 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 6 }, # 菊地真
      { idol_id: 39 }, # 馬場このみ
    ]
  },
  { name: '鬼だらけ！アイドル節分パニック',
    event_type: 'raid_event',
    started_at: '2015-01-29 17:00:00 +0900',
    ended_at: '2015-02-08 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 31 }, # 天空橋朋花
    ]
  },
  { name: '甘ふわ♪ショコラハウス',
    event_type: 'choco_event',
    started_at: '2015-02-10 17:00:00 +0900',
    ended_at: '2015-02-18 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 47 }, # 永吉昴
    ]
  },
  { name: 'アイドルマスターズカップ9',
    event_type: 'imc_event',
    started_at: '2015-02-20 17:00:00 +0900',
    ended_at: '2015-02-24 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 16 }, # 伊吹翼
    ]
  },
  { name: '繚乱！紅梅白梅ファッションショー',
    event_type: 'raid_event',
    started_at: '2015-02-27 17:00:00 +0900',
    ended_at: '2015-03-08 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 15 }, # 最上静香
    ]
  },
  { name: '熱踏！アイドルカーニバル',
    event_type: 'lesson_event',
    started_at: '2015-03-10 17:00:00 +0900',
    ended_at: '2015-03-17 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 18 }, # 島原エレナ
    ]
  },
  { name: '収束！プラチナスターライブ FINAL',
    event_type: 'psl_event',
    started_at: '2015-03-19 17:00:00 +0900',
    ended_at: '2015-03-29 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 3 }, # 星井美希
      { idol_id: 45 }, # 篠宮可憐
    ]
  },
  { name: '駆けろ！アイドルバスケリーグ',
    event_type: 'raid_event',
    started_at: '2015-04-07 17:00:00 +0900',
    ended_at: '2015-04-19 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 40 }, # 大神環
    ]
  },
  { name: 'アイドルマスターズカップ10',
    event_type: 'imc_event',
    started_at: '2015-04-22 17:00:00 +0900',
    ended_at: '2015-04-26 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 39 }, # 馬場このみ
    ]
  },
  { name: '【復刻】風雲！アイドルキャッスル！',
    event_type: 'raid_event',
    started_at: '2015-04-28 17:00:00 +0900',
    ended_at: '2015-05-06 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 4 }, # 萩原雪歩
    ]
  },
  { name: '特番！生っすか！？サンデー×50',
    event_type: 'lesson_event',
    started_at: '2015-05-08 17:00:00 +0900',
    ended_at: '2015-05-17 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 21 }, # 徳川まつり
    ]
  },
  { name: '絢爛！ゴールデンキャッスルライブ',
    event_type: 'caravan_event',
    started_at: '2015-05-19 17:00:00 +0900',
    ended_at: '2015-05-27 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 17 }, # 田中琴葉
    ]
  },
  { name: 'ENJOY H@RMONY!!',
    event_type: 'hhp_event',
    started_at: '2015-05-29 17:00:00 +0900',
    ended_at: '2015-06-07 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 14 }, # 春日未来
      { idol_id: 15 }, # 最上静香
      { idol_id: 16 }, # 伊吹翼
      { idol_id: 19 }, # 佐竹美奈子
      { idol_id: 20 }, # 所恵美
      { idol_id: 22 }, # 箱崎星梨花
      { idol_id: 24 }, # 望月杏奈
      { idol_id: 26 }, # 七尾百合子
      { idol_id: 27 }, # 高山紗代子
      { idol_id: 28 }, # 松田亜利沙
      { idol_id: 29 }, # 高坂海美
      { idol_id: 31 }, # 天空橋朋花
      { idol_id: 33 }, # 北沢志保
      { idol_id: 34 }, # 舞浜歩
      { idol_id: 36 }, # 矢吹可奈
      { idol_id: 37 }, # 横山奈緒
      { idol_id: 39 }, # 馬場このみ
      { idol_id: 41 }, # 豊川風花
      { idol_id: 45 }, # 篠宮可憐
      { idol_id: 46 }, # 百瀬莉緒
      { idol_id: 50 }, # ジュリア
    ]
  },
  { name: '夏到来！アイドル水上大運動会2015',
    event_type: 'choco_event',
    started_at: '2015-06-09 17:00:00 +0900',
    ended_at: '2015-06-17 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 33 }, # 北沢志保
    ]
  },
  { name: 'アイドルマスターズカップ11',
    event_type: 'imc_event',
    started_at: '2015-06-19 17:00:00 +0900',
    ended_at: '2015-06-23 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 13 }, # 我那覇響
    ]
  },
  { name: '【復刻】大合戦！戦国アイドル活劇',
    event_type: 'raid_event',
    started_at: '2015-06-25 17:00:00 +0900',
    ended_at: '2015-07-05 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 39 }, # 馬場このみ
    ]
  },
  { name: 'キラキラ★アイドル七夕祭',
    event_type: 'lesson_event',
    started_at: '2015-07-07 17:00:00 +0900',
    ended_at: '2015-07-15 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 48 }, # 北上麗花
    ]
  },
  { name: '壮麗！ブルーオーシャンライブ',
    event_type: 'caravan_event',
    started_at: '2015-07-17 17:00:00 +0900',
    ended_at: '2015-07-26 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 16 }, # 伊吹翼
      { idol_id: 38 }, # 二階堂千鶴
    ]
  },
  { name: 'はじける汗！アイドルビーチバレー大会',
    event_type: 'raid_event',
    started_at: '2015-07-28 17:00:00 +0900',
    ended_at: '2015-08-05 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 29 }, # 高坂海美
    ]
  },
  { name: '怨霊！アイドル肝試しホテル',
    event_type: 'choco_event',
    started_at: '2015-08-07 17:00:00 +0900',
    ended_at: '2015-08-17 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 50 }, # ジュリア
    ]
  },
  { name: 'アイドルマスターズカップ12',
    event_type: 'imc_event',
    started_at: '2015-08-19 17:00:00 +0900',
    ended_at: '2015-08-23 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 3 }, # 星井美希
    ]
  },
  { name: '【復刻】夏直前！アイドル水上大運動会！',
    event_type: 'raid_event',
    started_at: '2015-08-25 17:00:00 +0900',
    ended_at: '2015-09-02 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 36 }, # 矢吹可奈
    ]
  },
  { name: '潜入！スパイシスターズ',
    event_type: 'lesson_event',
    started_at: '2015-09-04 17:00:00 +0900',
    ended_at: '2015-09-15 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 15 }, # 最上静香
    ]
  },
  { name: '極楽♪温泉御殿ライブ',
    event_type: 'caravan_event',
    started_at: '2015-09-18 17:00:00 +0900',
    ended_at: '2015-09-29 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 1 }, # 天海春香
      { idol_id: 6 }, # 菊地真
    ]
  },
  { name: 'トリート♪トワイライトパレス',
    event_type: 'raid_event',
    started_at: '2015-10-02 17:00:00 +0900',
    ended_at: '2015-10-12 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 5 }, # 高槻やよい
    ]
  },
  { name: 'ようこそ♪聖ミリオン女学園',
    event_type: 'choco_event',
    started_at: '2015-10-14 17:00:00 +0900',
    ended_at: '2015-10-25 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 10 }, # 三浦あずさ
    ]
  },
  { name: 'アイドルマスターズカップ13',
    event_type: 'imc_event',
    started_at: '2015-10-28 17:00:00 +0900',
    ended_at: '2015-11-01 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 2 }, # 如月千早
    ]
  },
  { name: '【復刻】熱唱！真夏のサマーライブフェス',
    event_type: 'raid_event',
    started_at: '2015-11-02 17:00:00 +0900',
    ended_at: '2015-11-08 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 12 }, # 双海真美
    ]
  },
  { name: '期間限定★アイドルカフェ',
    event_type: 'lesson_event',
    started_at: '2015-11-10 17:00:00 +0900',
    ended_at: '2015-11-17 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 38 }, # 二階堂千鶴
    ]
  },
  { name: '飛翔！エアポートライブ',
    event_type: 'caravan_event',
    started_at: '2015-11-19 18:00:00 +0900',
    ended_at: '2015-11-29 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 31 }, # 天空橋朋花
      { idol_id: 32 }, # エミリー
    ]
  },
  { name: '大乱闘！サンタだらけの雪合戦',
    event_type: 'raid_event',
    started_at: '2015-12-01 17:00:00 +0900',
    ended_at: '2015-12-09 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 9 }, # 秋月律子
    ]
  },
  { name: 'アイドルマスターズカップ14',
    event_type: 'imc_event',
    started_at: '2015-12-11 17:00:00 +0900',
    ended_at: '2015-12-15 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 44 }, # 真壁瑞希
    ]
  },
  { name: '【復刻】恐怖！アイドルドキドキ肝だめし',
    event_type: 'raid_event',
    started_at: '2015-12-17 17:00:00 +0900',
    ended_at: '2015-12-23 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 19 }, # 佐竹美奈子
    ]
  },
  { name: 'アイドルスペースウォーズ EpisodeIV',
    event_type: 'choco_event',
    started_at: '2015-12-25 17:00:00 +0900',
    ended_at: '2016-01-04 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 32 }, # エミリー
    ]
  },
  { name: '大ヒット！アイドル商品開発部',
    event_type: 'lesson_event',
    started_at: '2016-01-08 17:00:00 +0900',
    ended_at: '2016-01-19 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 37 }, # 横山奈緒
    ]
  },
  { name: '光彩！スノーファンタジーライブ',
    event_type: 'caravan_event',
    started_at: '2016-01-21 17:00:00 +0900',
    ended_at: '2016-02-01 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 4 }, # 萩原雪歩
      { idol_id: 27 }, # 高山紗代子
    ]
  },
  { name: '芸術！？バレンタインミュージアム',
    event_type: 'choco_event',
    started_at: '2016-02-05 17:00:00 +0900',
    ended_at: '2016-02-16 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 15 }, # 最上静香
    ]
  },
  { name: 'アイドルマスターズカップ15',
    event_type: 'imc_event',
    started_at: '2016-02-19 17:00:00 +0900',
    ended_at: '2016-02-23 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 25 }, # ロコ
    ]
  },
  { name: '踊れ！アイドルダンスバトル',
    event_type: 'raid_event',
    started_at: '2016-02-27 17:00:00 +0900',
    ended_at: '2016-03-06 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 34 }, # 舞浜歩
    ]
  },
  { name: '守れ！3匹の子豚ハウス',
    event_type: 'lesson_event',
    started_at: '2016-03-09 17:00:00 +0900',
    ended_at: '2016-03-17 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 27 }, # 高山紗代子
    ]
  },
  { name: '【復刻】狂騒！ハロウィンフェスタ',
    event_type: 'raid_event',
    started_at: '2016-03-18 17:00:00 +0900',
    ended_at: '2016-03-23 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 1 }, # 天海春香
    ]
  },
  { name: '最高潮！メトロポリスライブ',
    event_type: 'caravan_event',
    started_at: '2016-03-25 17:00:00 +0900',
    ended_at: '2016-04-04 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 37 }, # 横山奈緒
      { idol_id: 30 }, # 中谷育
    ]
  },
  { name: 'アイドルマスターズカップ16',
    event_type: 'imc_event',
    started_at: '2016-04-06 17:00:00 +0900',
    ended_at: '2016-04-10 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 14 }, # 春日未来
    ]
  },
  { name: 'もっと！輝け！アイドル強化合宿',
    event_type: 'choco_event',
    started_at: '2016-04-12 17:00:00 +0900',
    ended_at: '2016-04-19 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 23 }, # 野々原茜
    ]
  },
  { name: 'BELIEVE MY DRE@M!!',
    event_type: 'hhp_event',
    started_at: '2016-04-22 17:00:00 +0900',
    ended_at: '2016-04-30 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 14 }, # 春日未来
      { idol_id: 15 }, # 最上静香
      { idol_id: 16 }, # 伊吹翼
      { idol_id: 17 }, # 田中琴葉
      { idol_id: 18 }, # 島原エレナ
      { idol_id: 19 }, # 佐竹美奈子
      { idol_id: 20 }, # 所恵美
      { idol_id: 21 }, # 徳川まつり
      { idol_id: 22 }, # 箱崎星梨花
      { idol_id: 23 }, # 野々原茜
      { idol_id: 24 }, # 望月杏奈
      { idol_id: 25 }, # ロコ
      { idol_id: 26 }, # 七尾百合子
      { idol_id: 27 }, # 高山紗代子
      { idol_id: 28 }, # 松田亜利沙
      { idol_id: 29 }, # 高坂海美
      { idol_id: 30 }, # 中谷育
      { idol_id: 31 }, # 天空橋朋花
      { idol_id: 32 }, # エミリー
      { idol_id: 33 }, # 北沢志保
      { idol_id: 34 }, # 舞浜歩
      { idol_id: 35 }, # 木下ひなた
      { idol_id: 36 }, # 矢吹可奈
      { idol_id: 37 }, # 横山奈緒
      { idol_id: 38 }, # 二階堂千鶴
      { idol_id: 39 }, # 馬場このみ
      { idol_id: 40 }, # 大神環
      { idol_id: 41 }, # 豊川風花
      { idol_id: 42 }, # 宮尾美也
      { idol_id: 43 }, # 福田のり子
      { idol_id: 44 }, # 真壁瑞希
      { idol_id: 45 }, # 篠宮可憐
      { idol_id: 46 }, # 百瀬莉緒
      { idol_id: 47 }, # 永吉昴
      { idol_id: 48 }, # 北上麗花
      { idol_id: 49 }, # 周防桃子
      { idol_id: 50 }, # ジュリア
    ]
  },
  { name: 'ワクワク！いなか暮らし',
    event_type: 'lesson_event',
    started_at: '2016-05-03 17:00:00 +0900',
    ended_at: '2016-05-09 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 30 }, # 中谷育
    ]
  },
  { name: 'ぽかぽか♪フラワーショーライブ',
    event_type: 'battle_event',
    started_at: '2016-05-10 17:00:00 +0900',
    ended_at: '2016-05-15 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 41 }, # 豊川風花
    ]
  },
  { name: 'アルティメットライブアリーナ 予選リーグ1',
    event_type: 'ula_event',
    started_at: '2016-05-16 17:00:00 +0900',
    ended_at: '2016-05-18 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 32 }, # エミリー
    ]
  },
  { name: '再点火！ロケットREスタートライブ!!',
    event_type: 'nplive_event',
    started_at: '2016-05-20 17:00:00 +0900',
    ended_at: '2016-05-26 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 22 }, # 箱崎星梨花
      { idol_id: 47 }, # 永吉昴
    ]
  },
  { name: 'アルティメットライブアリーナ 予選リーグ2',
    event_type: 'ula_event',
    started_at: '2016-05-27 17:00:00 +0900',
    ended_at: '2016-05-29 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 8 }, # 四条貴音
    ]
  },
  { name: '【復刻】年またぎ！アイドル紅白歌祭り',
    event_type: 'battle_event',
    started_at: '2016-05-31 17:00:00 +0900',
    ended_at: '2016-06-08 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 9 }, # 秋月律子
    ]
  },
  { name: 'アルティメットライブアリーナ 予選リーグ3',
    event_type: 'ula_event',
    started_at: '2016-06-10 17:00:00 +0900',
    ended_at: '2016-06-12 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 38 }, # 二階堂千鶴
    ]
  },
  { name: '囚われ！アイドルプリズン',
    event_type: 'choco_event',
    started_at: '2016-06-13 17:00:00 +0900',
    ended_at: '2016-06-20 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 28 }, # 松田亜利沙
    ]
  },
  { name: 'アイドルマスターズカップ エボリューション',
    event_type: 'imce_event',
    started_at: '2016-06-22 17:00:00 +0900',
    ended_at: '2016-06-26 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 46 }, # 百瀬莉緒
    ]
  },
  { name: 'アルティメットライブアリーナ 予選リーグ4',
    event_type: 'ula_event',
    started_at: '2016-06-27 17:00:00 +0900',
    ended_at: '2016-06-29 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 40 }, # 大神環
    ]
  },
  { name: 'アイドルヒーローズ リベンジ',
    event_type: 'battle_event',
    started_at: '2016-06-30 17:00:00 +0900',
    ended_at: '2016-07-07 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 17 }, # 田中琴葉
    ]
  },
  { name: 'アルティメットライブアリーナ 予選リーグ5',
    event_type: 'ula_event',
    started_at: '2016-07-08 17:00:00 +0900',
    ended_at: '2016-07-10 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 7 }, # 水瀬伊織
    ]
  },
  { name: 'ワッショイ！アイドル夏祭り',
    event_type: 'lesson_event',
    started_at: '2016-07-12 17:00:00 +0900',
    ended_at: '2016-07-19 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 43 }, # 福田のり子
    ]
  },
  { name: '夏色☆チアガールライブ',
    event_type: 'nplive_event',
    started_at: '2016-07-21 17:00:00 +0900',
    ended_at: '2016-07-28 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 11 }, # 双海亜美
      { idol_id: 50 }, # ジュリア
    ]
  },
  { name: 'アルティメットライブアリーナ 予選リーグ6',
    event_type: 'ula_event',
    started_at: '2016-07-29 17:00:00 +0900',
    ended_at: '2016-07-31 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 49 }, # 周防桃子
    ]
  },
  { name: 'Get to the top!サマースポーツフェス',
    event_type: 'choco_event',
    started_at: '2016-08-01 17:00:00 +0900',
    ended_at: '2016-08-08 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 21 }, # 徳川まつり
    ]
  },
  { name: 'おいで…。アイドル肝だめし遊園地',
    event_type: 'battle_event',
    started_at: '2016-08-09 17:00:00 +0900',
    ended_at: '2016-08-16 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 40 }, # 大神環
    ]
  },
  { name: '第2回アイドルマスターズカップ エボリューション',
    event_type: 'imce_event',
    started_at: '2016-08-18 17:00:00 +0900',
    ended_at: '2016-08-22 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 20 }, # 所恵美
    ]
  },
  { name: 'アルティメットライブアリーナ 予選リーグ7',
    event_type: 'ula_event',
    started_at: '2016-08-23 17:00:00 +0900',
    ended_at: '2016-08-25 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 33 }, # 北沢志保
    ]
  },
  { name: '華麗！ジェントルレディライブ',
    event_type: 'nplive_event',
    started_at: '2016-08-26 17:00:00 +0900',
    ended_at: '2016-09-01 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 29 }, # 高坂海美
      { idol_id: 49 }, # 周防桃子
    ]
  },
  { name: 'アルティメットライブアリーナ 予選リーグ8',
    event_type: 'ula_event',
    started_at: '2016-09-02 17:00:00 +0900',
    ended_at: '2016-09-04 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 36 }, # 矢吹可奈
    ]
  },
  { name: '【復刻】招福！アイドル干支レース',
    event_type: 'battle_event',
    started_at: '2016-09-05 17:00:00 +0900',
    ended_at: '2016-09-12 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 6 }, # 菊地真
    ]
  },
  { name: 'アルティメットライブアリーナ 予選リーグ9',
    event_type: 'ula_event',
    started_at: '2016-09-13 17:00:00 +0900',
    ended_at: '2016-09-15 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 19 }, # 佐竹美奈子
    ]
  },
  { name: '繋げ！アイドルトライアスロン',
    event_type: 'lesson_event',
    started_at: '2016-09-16 17:00:00 +0900',
    ended_at: '2016-09-22 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 2 }, # 如月千早
    ]
  },
  { name: '幕末！アイドル新撰組',
    event_type: 'battle_event',
    started_at: '2016-09-23 17:00:00 +0900',
    ended_at: '2016-10-02 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 33 }, # 北沢志保
    ]
  },
  { name: 'アルティメットライブアリーナ 予選リーグ10',
    event_type: 'ula_event',
    started_at: '2016-10-03 17:00:00 +0900',
    ended_at: '2016-10-05 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 16 }, # 伊吹翼
    ]
  },
  { name: '秋を満喫！ミリオンオータムフェア',
    event_type: 'choco_event',
    started_at: '2016-10-06 17:00:00 +0900',
    ended_at: '2016-10-16 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 18 }, # 島原エレナ
    ]
  },
  { name: 'アルティメットライブアリーナ チャレンジステージ',
    event_type: 'ula_event',
    started_at: '2016-10-17 17:00:00 +0900',
    ended_at: '2016-10-20 23:59:59 +0900'
  },
  { name: 'アルティメットライブアリーナ ファイナルリーグ',
    event_type: 'ula_event',
    started_at: '2016-10-21 17:00:00 +0900',
    ended_at: '2016-10-30 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 9 }, # 秋月律子
      { idol_id: 44 }, # 真壁瑞希
      { idol_id: 23 }, # 野々原茜
      { idol_id: 50 }, # ジュリア
      { idol_id: 28 }, # 松田亜利沙
      { idol_id: 42 }, # 宮尾美也
      { idol_id: 10 }, # 三浦あずさ
      { idol_id: 12 }, # 双海真美
      { idol_id: 27 }, # 高山紗代子
      { idol_id: 46 }, # 百瀬莉緒
    ]
  },
  { name: '【復刻】大激闘！765プロ野球',
    event_type: 'choco_event',
    started_at: '2016-11-08 12:00:00 +0900',
    ended_at: '2016-11-14 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 26 }, # 七尾百合子
    ]
  },
  { name: '第3回アイドルマスターズカップ エボリューション',
    event_type: 'imce_event',
    started_at: '2016-11-17 12:00:00 +0900',
    ended_at: '2016-11-20 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 35 }, # 木下ひなた
    ]
  },
  { name: '【復刻】大戦乱！アイドル三国志',
    event_type: 'battle_event',
    started_at: '2016-11-22 12:00:00 +0900',
    ended_at: '2016-11-30 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 13 }, # 我那覇響
    ]
  },
  { name: '陽だまり☆Sunshine Rhythm!!',
    event_type: 'budokan_event',
    started_at: '2016-12-02 12:00:00 +0900',
    ended_at: '2016-12-14 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 36 }, # 矢吹可奈
      { idol_id: 43 }, # 福田のり子
    ]
  },
  { name: '取り戻せ！サンタたちのプリンセス',
    event_type: 'battle_event',
    started_at: '2016-12-16 12:00:00 +0900',
    ended_at: '2016-12-25 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 8 }, # 四条貴音
    ]
  },
  { name: 'みんなで年越し！生っすか！？レボリューション×50',
    event_type: 'choco_event',
    started_at: '2016-12-27 12:00:00 +0900',
    ended_at: '2017-01-04 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 42 }, # 宮尾美也
    ]
  },
  { name: '月明かり☆BlueMoon Harmony!!',
    event_type: 'budokan_event',
    started_at: '2017-01-06 12:00:00 +0900',
    ended_at: '2017-01-18 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 20 }, # 所恵美
      { idol_id: 48 }, # 北上麗花
    ]
  },
  { name: '第4回アイドルマスターズカップ エボリューション',
    event_type: 'imce_event',
    started_at: '2017-01-20 12:00:00 +0900',
    ended_at: '2017-01-23 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 37 }, # 横山奈緒
    ]
  },
  { name: '奮闘！はじめてのアルバイト',
    event_type: 'lesson_event',
    started_at: '2017-01-25 12:00:00 +0900',
    ended_at: '2017-02-01 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 7 }, # 水瀬伊織
    ]
  },
  { name: '夢の国！？ショコラティエの大冒険',
    event_type: 'battle_event',
    started_at: '2017-02-03 12:00:00 +0900',
    ended_at: '2017-02-12 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 24 }, # 望月杏奈
    ]
  },
  { name: '星の煌めき☆Starlight Melody!!',
    event_type: 'budokan_event',
    started_at: '2017-02-14 12:00:00 +0900',
    ended_at: '2017-02-26 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 14 }, # 春日未来
      { idol_id: 41 }, # 豊川風花
    ]
  },
  { name: '4周年!!ミリオンメモリアルミュージカル',
    event_type: 'battle_event',
    started_at: '2017-02-28 12:00:00 +0900',
    ended_at: '2017-03-07 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 45 }, # 篠宮可憐
    ]
  },
  { name: 'みんなに届け！765PRO ALLSTARSライブ',
    event_type: 'choco_event',
    started_at: '2017-03-09 12:00:00 +0900',
    ended_at: '2017-03-15 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 1 }, # 天海春香
      { idol_id: 13 }, # 我那覇響
      { idol_id: 11 }, # 双海亜美
    ]
  },
  { name: 'TH@NK YOU for SMILE!!',
    event_type: 'hhp_event',
    started_at: '2017-03-17 12:00:00 +0900',
    ended_at: '2017-03-26 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 14 }, # 春日未来
      { idol_id: 15 }, # 最上静香
      { idol_id: 16 }, # 伊吹翼
      { idol_id: 17 }, # 田中琴葉
      { idol_id: 18 }, # 島原エレナ
      { idol_id: 19 }, # 佐竹美奈子
      { idol_id: 20 }, # 所恵美
      { idol_id: 21 }, # 徳川まつり
      { idol_id: 22 }, # 箱崎星梨花
      { idol_id: 23 }, # 野々原茜
      { idol_id: 24 }, # 望月杏奈
      { idol_id: 25 }, # ロコ
      { idol_id: 26 }, # 七尾百合子
      { idol_id: 27 }, # 高山紗代子
      { idol_id: 28 }, # 松田亜利沙
      { idol_id: 29 }, # 高坂海美
      { idol_id: 30 }, # 中谷育
      { idol_id: 31 }, # 天空橋朋花
      { idol_id: 32 }, # エミリー
      { idol_id: 33 }, # 北沢志保
      { idol_id: 34 }, # 舞浜歩
      { idol_id: 35 }, # 木下ひなた
      { idol_id: 36 }, # 矢吹可奈
      { idol_id: 37 }, # 横山奈緒
      { idol_id: 38 }, # 二階堂千鶴
      { idol_id: 39 }, # 馬場このみ
      { idol_id: 40 }, # 大神環
      { idol_id: 41 }, # 豊川風花
      { idol_id: 42 }, # 宮尾美也
      { idol_id: 43 }, # 福田のり子
      { idol_id: 44 }, # 真壁瑞希
      { idol_id: 45 }, # 篠宮可憐
      { idol_id: 46 }, # 百瀬莉緒
      { idol_id: 47 }, # 永吉昴
      { idol_id: 48 }, # 北上麗花
      { idol_id: 49 }, # 周防桃子
      { idol_id: 50 }, # ジュリア
    ]
  },
  { name: '【復刻】集まれ！アイドルモンスターズ',
    event_type: 'battle_event',
    started_at: '2017-04-04 12:00:00 +0900',
    ended_at: '2017-04-09 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 27 }, # 高山紗代子
    ]
  },
  { name: 'アイドルヒーローズ　サイドストーリー',
    event_type: 'lesson_event',
    started_at: '2017-04-11 12:00:00 +0900',
    ended_at: '2017-04-18 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 16 }, # 伊吹翼
    ]
  },
  { name: '第5回アイドルマスターズカップ エボリューション',
    event_type: 'imce_event',
    started_at: '2017-04-20 12:00:00 +0900',
    ended_at: '2017-04-23 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 38 }, # 二階堂千鶴
    ]
  },
  { name: 'ごきげんよう♪聖ミリオン女学園',
    event_type: 'choco_event',
    started_at: '2017-04-26 12:00:00 +0900',
    ended_at: '2017-05-07 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 2 }, # 如月千早
    ]
  },
  { name: '見つけ出せ！トレジャーハンターと伝説の財宝',
    event_type: 'battle_event',
    started_at: '2017-05-09 12:00:00 +0900',
    ended_at: '2017-05-16 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 39 }, # 馬場このみ
    ]
  },
  { name: 'Precious Days!ミリオンシアターライブ Day1',
    event_type: 'theater_event',
    started_at: '2017-05-18 12:00:00 +0900',
    ended_at: '2017-05-28 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 15 }, # 最上静香
    ]
  },
  { name: '【復刻】逆襲！グレートアイドルキャッスル',
    event_type: 'battle_event',
    started_at: '2017-05-30 12:00:00 +0900',
    ended_at: '2017-06-06 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 3 }, # 星井美希
    ]
  },
  { name: '第6回アイドルマスターズカップ エボリューション',
    event_type: 'imce_event',
    started_at: '2017-06-08 12:00:00 +0900',
    ended_at: '2017-06-11 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 1 }, # 天海春香
    ]
  },
  { name: '【復刻】祝祭！クリスマスフェスタ',
    event_type: 'choco_event',
    started_at: '2017-06-13 12:00:00 +0900',
    ended_at: '2017-06-21 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 47 }, # 永吉昴
    ]
  },
  { name: 'いざ勝負！真剣すごろくゲーム',
    event_type: 'battle_event',
    started_at: '2017-06-23 12:00:00 +0900',
    ended_at: '2017-07-02 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 12 }, # 双海真美
    ]
  },
  { name: '女子力UP！エンジョイホリデー♪',
    event_type: 'lesson_event',
    started_at: '2017-07-04 12:00:00 +0900',
    ended_at: '2017-07-12 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 46 }, # 百瀬莉緒
    ]
  },
  { name: 'Precious Days!ミリオンシアターライブ Day2',
    event_type: 'theater_event',
    started_at: '2017-07-14 12:00:00 +0900',
    ended_at: '2017-07-25 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 49 }, # 周防桃子
    ]
  },
  { name: '【復刻】激動！アイドル警察24時',
    event_type: 'battle_event',
    started_at: '2017-07-28 12:00:00 +0900',
    ended_at: '2017-08-07 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 17 }, # 田中琴葉
    ]
  },
  { name: '【復刻】甘ふわ♪ショコラハウス',
    event_type: 'choco_event',
    started_at: '2017-08-10 12:00:00 +0900',
    ended_at: '2017-08-21 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 21 }, # 徳川まつり
    ]
  },
  { name: '悪夢…呪われた人形の館',
    event_type: 'battle_event',
    started_at: '2017-08-24 12:00:00 +0900',
    ended_at: '2017-09-04 23:59:59 +0900',
    prizes_attributes: [
      { idol_id: 28 }, # 松田亜利沙
    ]
  }
])
